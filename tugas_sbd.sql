...
DELIMITER //
CREATE FUNCTION get_bobot(nilai_huruf VARCHAR(2))
RETURNS DECIMAL(3, 2)
DETERMINISTIC
BEGIN
    DECLARE bobot DECIMAL(3, 2);
    CASE 
        WHEN nilai_huruf = 'A' THEN SET bobot = 4.0;
        WHEN nilai_huruf = 'AB' THEN SET bobot = 3.5;
        WHEN nilai_huruf = 'B' THEN SET bobot = 3.0;
        WHEN nilai_huruf = 'BC' THEN SET bobot = 2.5;
        WHEN nilai_huruf = 'C' THEN SET bobot = 2.0;
        WHEN nilai_huruf = 'D' THEN SET bobot = 1.0;
        WHEN nilai_huruf = 'E' THEN SET bobot = 0.0;
        ELSE SET bobot = 0.0;
    END CASE;
    RETURN bobot;
END;
...
DELIMITER //
CREATE FUNCTION hitung_ipk(nim_input VARCHAR(14))
RETURNS DECIMAL(4, 2)
DETERMINISTIC
BEGIN
    ...
    -- Menghitung total bobot untuk mahasiswa
    SELECT SUM(get_bobot(na_huruf) * mk.sks) INTO total_bobot
    FROM nilai_smt ns
    JOIN matakuliah mk ON ns.kode_mk = mk.kode_mk
    WHERE ns.nim = nim_input;

    -- Menghitung total sks untuk mahasiswa
    ...
    -- Menghitung IPK
    IF total_sks > 0 THEN
        SET ipk = total_bobot / total_sks;
    ELSE
        SET ipk = 0; -- To handle division by zero
    END IF;
